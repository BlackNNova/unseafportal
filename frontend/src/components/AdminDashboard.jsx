...
const AdminDashboard = () => {
  console.log('🧪 TEST: AdminDashboard component loaded - v4.0-RLS-FIXED');
  ...
  const [financialStats, setFinancialStats] = useState({
    ...
    total_transfers: 0,
    total_withdrawals: 0,
    total_project_payments: 0,
    transaction_volume: 0,
    transfer_volume: 0,
    withdrawal_volume: 0,
    project_payment_volume: 0,
    recent_transactions: [],
    recent_transfers: [],
    recent_withdrawals: [],
    recent_project_payments: []
  });
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      console.log('🔍 SEARCH: Debounced search triggered for:', searchTerm, 'with filter:', userStatusFilter);
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      try {
        ...
        console.log('🧪 TEST: Starting dashboard load...');
        ...
        console.log('🧪 TEST: Table verification results:', tableResults);
        ...
        console.log('🧪 TEST: Dashboard load complete!');
      } catch (error) {
        console.error('🧪 TEST: Dashboard load error:', error);
  ...
  const fetchAdminData = async () => {
    console.log('🧪 TEST: fetchAdminData - Starting...');
    try {
      // Get current session
      console.log('🧪 TEST: fetchAdminData - Checking session...');
      const { data: { session }, error: sessionError } = await supabase.auth.getSession();
      
      if (sessionError || !session) {
        console.log('🧪 TEST: fetchAdminData - No session, checking localStorage...');
        // Try to get from localStorage as fallback
        const adminData = localStorage.getItem('admin');
        if (adminData) {
          console.log('🧪 TEST: fetchAdminData - Found admin in localStorage');
          setAdmin(JSON.parse(adminData));
        } else {
          console.log('🧪 TEST: fetchAdminData - No admin data found anywhere');
        }
        return;
      }

      console.log('🧪 TEST: fetchAdminData - Session found:', session.user.email);
      ...
      if (profileError) {
        console.error('🧪 TEST: fetchAdminData - Profile error:', profileError);
        throw new Error('Admin profile not found');
      }
      
      console.log('🧪 TEST: fetchAdminData - Success:', adminProfile);
      setAdmin(adminProfile);
    } catch (err) {
      console.error('🧪 TEST: fetchAdminData - ERROR:', err);
      setError('Failed to fetch admin data');
    }
  };

  const fetchStats = async () => {
    console.log('🧪 TEST: fetchStats - Starting...');
    try {
      // Get user count
      console.log('🧪 TEST: fetchStats - Counting total users...');
      ...
      const statsObject = {
        ...
        active_users: approvedUsers || 0, // 🔧 FIX: Use approvedUsers for accurate count
      ...
      console.log('🧪 TEST: Stats object structure:', statsObject);
      console.log('🧪 TEST: Property names:', Object.keys(statsObject));
  ...
  const fetchFinancialStats = async () => {
    console.log('🧪 TEST: fetchFinancialStats - Starting...');
    try {
      ...
      // Fetch withdrawals data - SIMPLIFIED QUERY
      ...
      if (withdrawalsData && withdrawalsData.length > 0) {
        ...
        withdrawalsWithUsers = withdrawalsData.map(w => {
      ...
      console.log('🔍 DEBUG: Raw withdrawals query:', { withdrawalsData, withdrawalsError, count: withdrawalsData?.length });
      console.log('🔍 DEBUG: Withdrawals with users:', { withdrawalsWithUsers, count: withdrawalsWithUsers?.length });

      // Fetch project_payments data - SIMPLIFIED QUERY
      ...
      // Get user info separately for project payments
      let projectPaymentsWithUsers = [];
      if (projectPaymentsData && projectPaymentsData.length > 0) {
        const paymentUserIds = projectPaymentsData.map(p => p.user_id);
        const { data: paymentUsersData } = await supabase
          .from('users')
          .select('id, first_name, last_name, email, account_number')
          .in('id', paymentUserIds);
          
        projectPaymentsWithUsers = projectPaymentsData.map(p => {
          const user = paymentUsersData?.find(u => u.id === p.user_id);
          return {
            ...p,
            users: user
          };
      ...
      console.log('🔍 DEBUG: Raw project_payments query:', { projectPaymentsData, projectPaymentsError, count: projectPaymentsData?.length });
      console.log('🔍 DEBUG: Project payments with users:', { projectPaymentsWithUsers, count: projectPaymentsWithUsers?.length });
      ...
      const { count: totalProjectPayments } = await supabase
        .from('project_payments')
        .select('*', { count: 'exact', head: true });
      ...
      const projectPaymentVolume = allProjectPayments?.reduce((sum, p) => sum + parseFloat(p.amount || 0), 0) || 0;
      ...
      // EMERGENCY BACKUP: Use raw withdrawal data if user join fails
      const dataToUse = withdrawalsWithUsers.length > 0 ? withdrawalsWithUsers : (withdrawalsData || []);
      
      const formattedWithdrawals = dataToUse.map(wd => ({
        ...wd,
        user_info: {
          full_name: `${wd.users?.first_name || ''} ${wd.users?.last_name || ''}`.trim() || 'Unknown User',
          email: wd.users?.email || 'unknown@email.com',
          account_number: wd.users?.account_number || 'N/A'
        }
      }));

      // EMERGENCY BACKUP: Use raw project payment data if user join fails
      ...
      const formattedProjectPayments = paymentDataToUse.map(p => ({
        ...p,
        user_info: {
          full_name: `${p.users?.first_name || ''} ${p.users?.last_name || ''}`.trim() || 'Unknown User',
      ...
      const financialData = {
        ...
        total_project_payments: totalProjectPayments || 0,
        transaction_volume: transactionVolume,
        transfer_volume: transferVolume,
        withdrawal_volume: withdrawalVolume,
        project_payment_volume: projectPaymentVolume,
        recent_transactions: formattedTransactions,
        recent_transfers: formattedTransfers,
        recent_withdrawals: formattedWithdrawals,
        recent_project_payments: formattedProjectPayments
      };

      setFinancialStats(financialData);
      console.log('🧪 TEST: Financial stats loaded:', financialData);
      console.log('🔍 DEBUG: Recent withdrawals:', financialData.recent_withdrawals);
  ...
  const fetchUsers = async (status = null) => {
    ...
    console.log('🧪 TEST: fetchUsers - Starting with status:', filterStatus, 'from:', status ? 'button click' : 'search/refresh');
    console.log('🧪 TEST: fetchUsers - Search term:', searchTerm);
    
    // Update the filter state if a new status is provided (from button clicks)
    if (status) {
      console.log('🧪 TEST: fetchUsers - Updating filter state from', userStatusFilter, 'to', status);
    ...
    try {
      ...
      console.log('🧪 TEST: fetchUsers - Building query...');
      ...
      console.log('🧪 TEST: fetchUsers - Raw users count:', usersData?.length);
      ...
      if (grantsError) {
        console.warn('🧪 TEST: fetchUsers - Grant fetch error:', grantsError);
      }
      ...
      console.log('🧪 TEST: fetchUsers - Sample user with grant:', usersWithGrants?.[0]);
      ...
      if (issues.length > 0) {
        console.warn('🧪 TEST: User property issues found:', issues);
      }
      
      const normalizedUsers = usersWithGrants.map(normalizeUser);
      console.log('🧪 TEST: fetchUsers - Normalized users count:', normalizedUsers.length);
      
      setUsers(normalizedUsers);
    } catch (err) {
      console.error('🧪 TEST: fetchUsers - ERROR:', err);
  ...
  const handleAccountAction = async (userId, action) => {
    try {
      // 🔧 FIX: Use 'approved' instead of 'active' to match database constraints
      ...
      // ✅ FIX: Refresh all related data to ensure UI consistency
      await Promise.all([
      ...
      try {
        await supabase
          ...
          .insert({
            ...
            old_value: 'unknown', // We don't have the old status in this context
            new_value: newStatus,
  ...
  const changeAccountStatus = async (user, newStatus) => {
    console.log('🆕 VERSION CHECK: UNSEAF-Portal-v4.0-RLS-FIXED');
    console.log('🔔 BUTTON CLICKED: changeAccountStatus called with:', { userId: user.id, newStatus, currentStatus: user.account_status });
    
    // 🐛 DEBUG: Check current admin auth status
    const { data: { session }, error: sessionError } = await supabase.auth.getSession();
    ...
    try {
      // 🐛 DEBUG: Log the exact update query being sent
      ...
      if (error) {
        ...
        if (error.code === 'PGRST116' || error.message?.includes('policy')) {
          console.error('🚫 RLS POLICY ERROR - Admin update blocked by Row Level Security!');
          alert(`❌ Permission denied: Admin update policies not configured correctly.\n\nError: ${error.message}\n\nPlease run the FIX_ADMIN_UPDATE_POLICIES.sql script in Supabase.`);
        } else {
      ...
      // 🐛 DEBUG: Verify the update actually happened
      console.log('✅ UPDATE SUCCESS - Records affected:', updateResult?.length || 0);
      if (updateResult && updateResult.length > 0) {
        console.log('📊 Updated record:', updateResult[0]);
        console.log('🔄 Status changed from', user.account_status, 'to', updateResult[0].account_status);
      }
      
      alert(`✅ Account ${successMessage[newStatus]} successfully`);
      
      // 🎯 SMART TAB NAVIGATION: Switch to the appropriate tab based on new status
      const targetTab = {
        'approved': 'approved',
        'active': 'approved',    // Legacy support
        'pending': 'pending',
      ...
      console.log(`🎯 SMART NAVIGATION: Switching to '${targetTab}' tab after status change to '${newStatus}'`);
      
      // ✅ FIX: Refresh all related data and switch to appropriate tab
      await Promise.all([
      ...
      let userMessage = `❌ Status change failed: ${err.message}`;
      
      if (err.code === 'PGRST116') {
        userMessage = `❌ Permission denied: Admin policies need to be configured.\n\nPlease contact the system administrator to run the RLS policy fix.`;
      }
  ...
  const healDuplicateTransactionNumbers = async () => {
    if (!confirm('This will automatically fix all duplicate transaction numbers. Continue?')) {
      return;
    }
    ...
    try {
      console.log('🔧 Starting duplicate transaction number healing...');
      ...
      console.log('✅ Healing complete:', data);
      alert(`✅ Successfully fixed ${fixedCount} duplicate transaction numbers!\n\nThe system is now consistent.`);
      
      // Refresh financial stats to show updated data
      await fetchFinancialStats();
      
    } catch (error) {
      console.error('🛑 Error healing duplicates:', error);
      alert(`Failed to heal duplicates: ${error.message}`);
    } finally {
...
Type 'DELETE' to confirm:`;
  ...
  {console.log('🚨 EMERGENCY: Admin Dashboard Render') || null}
  {console.log('🚨 EMERGENCY: activeTab =', activeTab) || null}
  {console.log('🚨 EMERGENCY: admin =', admin) || null}
  {console.log('🚨 EMERGENCY: loading =', loading) || null}
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex space-x-4 mb-6 overflow-x-auto">
          ...
          <Button
            onClick={() => {
              console.log('🚨 EMERGENCY: Financial Activity button clicked!');
              console.log('🚨 EMERGENCY: Setting activeTab to financial');
              setActiveTab('financial');
              console.log('🚨 EMERGENCY: activeTab should now be financial');
            }}
        ...
        {activeTab === 'financial' && (
          <div className="space-y-6">
            {console.log('🚨 EMERGENCY DEBUG: Financial Activity Tab Loaded!') || null}
            {console.log('🚨 EMERGENCY DEBUG: financialStats =', financialStats) || null}
            {console.log('🚨 EMERGENCY DEBUG: recent_withdrawals =', financialStats?.recent_withdrawals) || null}
            {console.log('🚨 EMERGENCY DEBUG: recent_withdrawals length =', financialStats?.recent_withdrawals?.length) || null}
            
            {/* System Maintenance - Heal Duplicates */}
              ...
              <div className="flex items-center justify-between w-full">
                <div className="flex-1">
                  ...
                  <AlertDescription className="text-blue-800">
                    Fix duplicate transaction numbers from legacy race conditions. This will regenerate unique numbers for duplicates while keeping data integrity.
                  </AlertDescription>
                </div>
                <Button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {/* Withdrawals Table - Responsive Card Layout */}
              <Card>
                <CardHeader>
                ...
                <CardContent>
                  {console.log('🔍 DEBUG: Rendering withdrawals:', financialStats.recent_withdrawals) || null}
                  {financialStats.recent_withdrawals && financialStats.recent_withdrawals.length > 0 ? (
                    <div className="space-y-4">
                      {financialStats.recent_withdrawals.map((withdrawal, index) => (
                        <div key={index} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                            {/* Left Section: User & Amount */}
                            <div className="flex-1 space-y-2">
                              <div className="flex items-start justify-between">
                                <div>
                                  <div className="text-sm font-semibold text-gray-900">{withdrawal.user_info?.full_name || 'Unknown'}</div>
                                ...
                                <Badge 
                                  className={
                                    ...
                                    withdrawal.status === 'failed' ? 'bg-red-600' : 'bg-gray-600'
                                  }
                                >
                              ...
                              <div className="flex items-baseline gap-4">
                                <div>
                                  ...
                                  <span className="text-lg font-bold text-red-600">${parseFloat(withdrawal.amount).toFixed(2)}</span>
                                </div>
                                <div>
                                  <span className="text-xs text-gray-500">Fee: </span>
                                  <span className="text-sm text-gray-700">${parseFloat(withdrawal.fee || 0).toFixed(2)}</span>
                                </div>
                                <div>
                                  <span className="text-xs text-gray-500">Method: </span>
                                  <span className="text-sm text-gray-700">{withdrawal.method || 'Bank Transfer'}</span>
                                </div>
                              </div>
                            </div>
                    ...
                    <div className="py-8 text-center text-gray-500">
                      <div>
                        🚨 <strong>DEBUG:</strong> No withdrawal data found<br/>
                        recent_withdrawals length: {financialStats?.recent_withdrawals?.length || 'undefined'}<br/>
                        financialStats exists: {financialStats ? 'YES' : 'NO'}<br/>
                        <button 
                          onClick={() => console.log('FULL financialStats:', financialStats)}
              ...
              {/* Project Payments Table - Responsive Card Layout */}
              <Card>
                <CardHeader>
                ...
                <CardContent>
                  {console.log('🔍 DEBUG: Rendering project payments:', financialStats.recent_project_payments) || null}
                      ...
                      {financialStats.recent_project_payments.map((payment, index) => (
                        <div key={index} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                            {/* Left Section: User, Recipient & Details */}
                            <div className="flex-1 space-y-3">
                              {/* Header Row */}
                              <div className="flex items-start justify-between">
                                <div className="flex-1">
                                  <div className="text-sm font-semibold text-gray-900">{payment.user_info?.full_name || 'Unknown'}</div>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                <div>
                                  ...
                                  <span className="text-sm font-medium text-gray-900">{payment.recipient_name}</span>
                                </div>
                                <div>
                                  <span className="text-xs text-gray-500">Transaction: </span>
                                  <span className="text-xs font-mono text-gray-600">{payment.transaction_number}</span>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-4 flex-wrap">
                                <div>
                                  <span className="text-xs text-gray-500">Amount: </span>
                                  <span className="text-lg font-bold text-purple-600">${parseFloat(payment.amount).toFixed(2)}</span>
                                </div>
                                <div>
                                  <span className="text-xs text-gray-500">Fee: </span>
                                  <span className="text-sm text-gray-700">${parseFloat(payment.fee || 0).toFixed(2)}</span>
                                </div>
                                <Badge className="bg-purple-100 text-purple-700 hover:bg-purple-100">
                                  {payment.category?.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
                                </Badge>
                              </div>
                            </div>
                      ...
                      <div key={index} className="border rounded-lg p-6 bg-red-50">
                        ...
                        <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded">
                          ...
                          <ul className="text-sm text-yellow-800 space-y-1">
                            <li>• <strong>Approve one</strong> and reject the others</li>
                            <li>• <strong>Delete unwanted accounts</strong> to free up the grant number</li>
                            <li>• <strong>Set legitimate account to approved</strong> and others to rejected</li>
                            <li>• Contact users directly to verify which registration is legitimate</li>
                          </ul>
                        </div>
                      </div>
                    ))}
