...
import { verifyUserPin } from '../utils/pinUtils';
...
const PinVerification = ({ 
  ...
  onClose, 
  onSuccess, 
  transactionDetails = {}, // Changed from transactionData
  loading = false 
}) => {
  ...
  const handleVerifyPin = async () => {
    ...
    setVerifying(true);
    setError('');
    console.log('🔐 PIN Verification: Starting verification for 6-digit PIN');

    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) {
        console.error('❌ PIN Verification: No authenticated user');
        setError('Please log in to continue');
        setVerifying(false);
        return;
      }

      console.log('✅ PIN Verification: User authenticated, userId:', user.id);
      console.log('🔍 PIN Verification: Calling verifyUserPin...');
      
      const result = await verifyUserPin(user.id, pin);
      console.log('📋 PIN Verification: Result received:', result);
      
      if (result.success) {
        console.log('✅ PIN Verification: Success! Calling onSuccess callback...');
    ...
    console.log('🔐 PIN Verification: Setting verifying to false');
    setVerifying(false);
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4 shadow-2xl">
        ...
        {/* Transaction Summary */}
        {transactionDetails && transactionDetails.amount > 0 && (
            ...
            <div className="space-y-2 text-sm">
              ...
              {transactionDetails.method && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium capitalize">{transactionDetails.method.replace('_', ' ')}</span>
                </div>
              )}
              {transactionDetails.fee && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-orange-600">
                    {formatCurrency(transactionDetails.fee)}
                  </span>
                </div>
              )}
              {(transactionDetails.amount - transactionDetails.fee) && (
                <div className="flex justify-between border-t pt-2">
                  <span className="text-gray-600 font-medium">You'll Receive:</span>
                  <span className="font-bold text-green-600">
                    {formatCurrency(transactionDetails.amount - transactionDetails.fee)}
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="flex justify-center relative h-12">
              <div className="flex space-x-2">
                {[...Array(6)].map((_, index) => (
                  <div 
                    key={index} 
                    className={`w-12 h-12 border-2 rounded-lg flex items-center justify-center text-2xl font-bold cursor-pointer ${
                      pin.length === 6 ? 'border-blue-500 bg-blue-50' : 'border-gray-300'
                    }`}
                    onClick={() => {
                      const input = document.querySelector('.pin-verification-input');
              ...
              <input
                ...
                className="pin-verification-input absolute opacity-0"
                style={{ 
                  top: 0, 
                  left: 0, 
                  width: '100%', 
